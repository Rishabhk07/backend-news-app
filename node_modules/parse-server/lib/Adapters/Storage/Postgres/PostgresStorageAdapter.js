...
var PostgresStorageAdapter = exports.PostgresStorageAdapter = function () {
  function PostgresStorageAdapter(_ref2) {
    ...
    var _createClient = (0, _PostgresClient.createClient)(uri, databaseOptions),
        client = _createClient.client,
        pgp = _createClient.pgp;
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function deleteClass(className) {
      ...
      var operations = [{ query: 'DROP TABLE IF EXISTS $1:name', values: [className] }, { query: 'DELETE FROM "_SCHEMA" WHERE "className" = $1', values: [className] }];
      return this._client.tx(function (t) {
        return t.none(_this4._pgp.helpers.concat(operations));
      }).then(function () {
    ...
    value: function deleteAllClasses() {
      ...
      return this._client.any('SELECT * FROM "_SCHEMA"').then(function (results) {
        ...
        var classes = ['_SCHEMA', '_PushStatus', '_JobStatus', '_JobSchedule', '_Hooks', '_GlobalConfig', '_Audience'].concat(_toConsumableArray(results.map(function (result) {
    ...
    value: function createObject(className, schema, object) {
      ...
      debug(qs, values);
      return this._client.none(qs, values).then(function () {
        return { ops: [object] };
      }).catch(function (error) {
    ...
    value: function updateObjectsByQuery(className, schema, query, update) {
      ...
      var _loop2 = function _loop2(_fieldName) {
        ...
        if (fieldValue === null) {
          ...
          values.push(_fieldName, fieldValue.longitude, fieldValue.latitude);
          index += 3;
      ...
      var whereClause = where.pattern.length > 0 ? 'WHERE ' + where.pattern : '';
      var qs = 'UPDATE $1:name SET ' + updatePatterns.join(',') + ' ' + whereClause + ' RETURNING *';
