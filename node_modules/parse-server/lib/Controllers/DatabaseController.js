  ...
  if (query.$or) {
    if (query.$or instanceof Array) {
      ...
      Object.keys(query).forEach(function (key) {
        ...
        var hasNears = false;
        if (query[key] != null && _typeof(query[key]) == 'object') {
...
DatabaseController.prototype.update = function (className, query, update) {
  ...
  return this.loadSchema().then(function (schemaController) {
    return (isMaster ? Promise.resolve() : schemaController.validatePermission(className, aclGroup, 'update')).then(function () {
      relationUpdates = _this4.collectRelationUpdates(className, originalQuery.objectId, update);
      if (!isMaster) {
      ...
      return _this4.handleRelationUpdates(className, originalQuery.objectId, update, relationUpdates).then(function () {
        return result;
      });
    }).then(function (result) {
      if (skipSanitization) {
...
DatabaseController.prototype.collectRelationUpdates = function (className, objectId, update) {
  ...
  var process = function process(op, key) {
    ...
    if (op.__op == 'Batch') {
      ...
      try {
        for (var _iterator = op.ops[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var x = _step.value;
  ...
  try {
    for (var _iterator2 = deleteMe[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
DatabaseController.prototype.handleRelationUpdates = function (className, objectId, update, ops) {
  ...
  ops.forEach(function (_ref4) {
    ...
    if (op.__op == 'RemoveRelation') {
      ...
      try {
        for (var _iterator4 = op.objects[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
...
DatabaseController.prototype.create = function (className, object) {
  ...
  var isMaster = acl === undefined;
  var aclGroup = acl || [];
  var relationUpdates = this.collectRelationUpdates(className, null, object);
  return this.validateClassName(className).then(function () {
    ...
    return (isMaster ? Promise.resolve() : schemaController.validatePermission(className, aclGroup, 'create')).then(function () {
      ...
      return _this7.handleRelationUpdates(className, null, object, relationUpdates).then(function () {
...
DatabaseController.prototype.reduceInRelation = function (className, query, schema) {
  ...
  var promises = Object.keys(query).map(function (key) {
    ...
    if (query[key] && (query[key]['$in'] || query[key]['$ne'] || query[key]['$nin'] || query[key].__type == 'Pointer')) {
      ...
      queries = Object.keys(query[key]).map(function (constraintKey) {
...
DatabaseController.prototype.find = function (className, query) {
  ...
  var _ref7 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
      ...
      sort = _ref7$sort === undefined ? {} : _ref7$sort,
      count = _ref7.count,
      keys = _ref7.keys,
      op = _ref7.op,
      readPreference = _ref7.readPreference;
      ...
      return (isMaster ? Promise.resolve() : schemaController.validatePermission(className, aclGroup, op)).then(function () {
        ...
        if (count) {
          if (!classExists) {
            ...
            return _this10.adapter.count(className, schema, query, readPreference);
          }
        } else {
          if (!classExists) {
            return [];
          } else {
            return _this10.adapter.find(className, schema, query, { skip: skip, limit: limit, sort: sort, keys: keys, readPreference: readPreference }).then(function (objects) {
...
var untransformObjectACL = function untransformObjectACL(_ref8) {
  var _rperm = _ref8._rperm,
      _wperm = _ref8._wperm,
      output = _objectWithoutProperties(_ref8, ['_rperm', '_wperm']);
